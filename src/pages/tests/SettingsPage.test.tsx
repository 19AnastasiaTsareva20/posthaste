...
describe("SettingsPage", () => {
  ...
  it("exports all notes", async () => {
    ...
    const exportButton = screen.getByRole("button", {
      name: /экспортировать все заметки/i,
    });
  ...
  it("imports notes from file", async () => {
    const user = userEvent.setup();
    const mockFile = new File(['{"notes": []}'], "notes.json", {
  ...
  it("validates imported file format", async () => {
    const user = userEvent.setup();
    const invalidFile = new File(["invalid json"], "notes.txt", {
      type: "text/plain",
    });
  ...
  it("changes default view mode", async () => {
    ...
    const viewModeSelect = screen.getByLabelText(
      /режим просмотра по умолчанию/i,
    );
    await user.selectOptions(viewModeSelect, "list");

    expect(localStorage.setItem).toHaveBeenCalledWith(
      "notesflow-default-view",
      "list",
    );
  });
  ...
  it("shows welcome modal toggle", async () => {
    ...
    const welcomeToggle = screen.getByRole("checkbox", {
      name: /показывать приветствие/i,
    });
    await user.click(welcomeToggle);

    expect(localStorage.setItem).toHaveBeenCalledWith(
      "notesflow-show-welcome",
      "false",
    );
  });
  ...
  it("handles storage errors gracefully", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/ошибка сохранения настроек/i),
      ).toBeInTheDocument();
    });
  });

...
describe("ViewArticlePage", () => {
  ...
  it("renders note content correctly", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("This is the note content with"),
      ).toBeInTheDocument();
  ...
  it("toggles favorite status", async () => {
    ...
    expect(mockedNoteStorage.toggleFavorite).toHaveBeenCalledWith(
      "test-note-1",
    );
  });
  ...
  it("shows favorite star when note is favorite", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: /удалить из избранного/i }),
      ).toBeInTheDocument();
    });
  });

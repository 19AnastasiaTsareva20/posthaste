...
export const ArchivePage: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'archivedAt' | 'updatedAt' | 'title'>('archivedAt');
    ...
    try {
      ...
      if (savedNotes) {
        ...
        const updatedNotes = notes.map((note: any) =>
          note.id === noteId ? { ...note, isArchived: false, archivedAt: undefined } : note
        );
  ...
  const handleDeleteNote = (noteId: string) => {
    if (window.confirm('Вы уверены, что хотите окончательно удалить эту заметку? Это действие нельзя отменить.')) {
      try {
        const savedNotes = localStorage.getItem('notesflow-notes');
        if (savedNotes) {
          const notes = JSON.parse(savedNotes);
  ...
  const handleClearArchive = () => {
    if (window.confirm('Вы уверены, что хотите окончательно удалить все архивированные заметки? Это действие нельзя отменить.')) {
      try {
        const savedNotes = localStorage.getItem('notesflow-notes');
        if (savedNotes) {
          const notes = JSON.parse(savedNotes);
            ...
            <>
              ...
              <div className="flex items-center justify-between">
                <div className="text-sm text-text-secondary dark:text-dark-text-secondary">
                  Заметки автоматически архивируются при удалении и хранятся здесь
                </div>
        ...
        {filteredAndSortedNotes.length === 0 ? (
          <Card className="p-12 text-center">
            <div className="w-16 h-16 bg-neutral-100 dark:bg-dark-surface rounded-full flex items-center justify-center mx-auto mb-4">
              <svg className="h-8 w-8 text-text-muted dark:text-dark-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 8l4 4m0 0l4-4m-4 4V3m0 14l-4-4m4 4l4-4" />
              </svg>
            </div>

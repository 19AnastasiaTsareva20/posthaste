...
export const TagManager: React.FC<TagManagerProps> = ({
  ...
  const popularTags = [
    'работа', 'личное', 'идеи', 'проект', 'важное', 'дела',
    'планы', 'заметки', 'учеба', 'здоровье', 'путешествия'
  ];
  ...
  const createTag = () => {
    ...
    const savedTagColors = JSON.parse(localStorage.getItem('notesflow-tag-colors') || '{}');
    savedTagColors[tagName] = selectedColor;
    localStorage.setItem('notesflow-tag-colors', JSON.stringify(savedTagColors));
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div 
                  ...
                  style={{ 
                    backgroundColor: selectedTag === tag.name ? 'white' : getTagColor(tag.name)
                  }}
                />
        ...
        {allTags.length === 0 && !isCreating && (
          <div className="text-center py-6 text-text-muted dark:text-dark-text-muted">
            <svg className="h-12 w-12 mx-auto mb-3 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M7 7h10v2l-2 2h-6l-2-2V7zM4 9h2v10a2 2 0 002 2h8a2 2 0 002-2V9h2V7H4v2z" />
            </svg>
            <p className="text-sm mb-3">Теги помогают быстро находить заметки</p>
            <Button
              size="sm"
              variant="outline"
              onClick={() => setIsCreating(true)}

...
export const SearchComponent: React.FC<SearchComponentProps> = ({ 
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (query.trim() && query.length > 2) {
        const recent = JSON.parse(localStorage.getItem('notesflow-recent-searches') || '[]');
        const updatedRecent = [query, ...recent.filter((item: string) => item !== query)].slice(0, 5);
        localStorage.setItem('notesflow-recent-searches', JSON.stringify(updatedRecent));
      }
    }, 300);
  ...
  const getRecentSearches = () => {
    return JSON.parse(localStorage.getItem('notesflow-recent-searches') || '[]');
  };
          ...
          <div className="absolute top-full left-0 right-0 mt-2 z-50">
            <Card className="py-2 animate-slide-up shadow-large">
              ...
              {filteredSuggestions.length > 0 && (
                <div className="mb-2">
                  ...
                  {filteredSuggestions.map((suggestion, index) => (
                    ...
                    >
                      <svg className="h-4 w-4 text-text-muted dark:text-dark-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                      </svg>
                      {suggestion}
                    </button>
              ...
              {recentItems.length > 0 && (
                <div>
                  {filteredSuggestions.length > 0 && <hr className="border-border dark:border-dark-border my-2" />}
                  <div className="px-4 py-2 text-xs font-semibold text-text-muted dark:text-dark-text-muted uppercase tracking-wide">
                    Недавние поиски
                  </div>
